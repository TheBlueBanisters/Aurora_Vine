/**
 * GPA ↔ 百分制 换算规则（与 renderer/modules/gpa-conversion.mjs 保持一致）。
 *
 * 5 分制锚点（用户指定规则）：
...
export function gpaConversionRuleText(scale) {
  const numericScale = scale === 4 || scale === '4' ? 4 : 5;
  const anchors = gpaScaleAnchors(numericScale);
  const pieces = anchors.map((a) => `${a.gpa.toFixed(1)}→${a.percent}`);
  if (numericScale === 5) {
    return {
      zh: `5 分制换算锚点：${pieces.join('，')}；线性公式 percent = 50 + GPA × 10。`,
      en: `5-scale anchor table: ${pieces.join(', ')}; linear formula percent = 50 + GPA × 10.`
    };
  }
  return {
    zh: `4 分制换算锚点：${pieces.join('，')}；位于锚点之间按线性插值。`,
    en: `4-scale anchor table: ${pieces.join(', ')}; linear interpolation between anchors.`
  };
}
