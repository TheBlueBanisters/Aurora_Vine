...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyAuthStateAndRefresh(payload, options = {}) {
    ...
    if (currentAuthState.mode === 'none') {
      ...
      if (document.body.classList.contains('landing-mode')) {
        ...
        maybeShowUsageGuideOnFirstEntry();
      }
      refreshAuthBoundUI();
    }
  }
  ...
  function positionUsageGuideCard(targetEl) {
    ...
    if (left + cardRect.width > viewportWidth - 24) {
      left = rect.left - cardRect.width - USAGE_GUIDE_CARD_GAP;
    }
    if (left < 24) left = 24;
    if (top < 24) top = 24;
    if (top + cardRect.height > viewportHeight - 24) {
      top = viewportHeight - cardRect.height - 24;
    }
  ...
  function runUsageGuideStep(stepIndex) {
    ...
    if (isFinal) {
      ...
      if (usageGuideNav) {
        ...
        requestAnimationFrame(() => {
          requestAnimationFrame(() => positionUsageGuideCard(usageGuideNav));
        });
      } else {
      ...
      if (targetNav) {
        ...
        requestAnimationFrame(() => {
          requestAnimationFrame(() => positionUsageGuideCard(targetNav));
        });
      } else {
        spotlight.style.display = 'none';
      }
  ...
  function closeUsageGuide(saveNoMore = false) {
    const overlay = document.getElementById('usage-guide-overlay');
    if (!overlay) return;
    ...
    if (saveNoMore) {
      try {
        localStorage.setItem(USAGE_GUIDE_NO_MORE_KEY, 'true');
      } catch (_) {}
    }
  }

  function startUsageGuide(manualReplay = false) {
  ...
  function initUsageGuide() {
    ...
    prevBtn.addEventListener('click', () => {
      if (usageGuideCurrentStep > 0) {
        usageGuideCurrentStep--;
        runUsageGuideStep(usageGuideCurrentStep);
      }
    });
    nextBtn.addEventListener('click', () => {
      if (usageGuideCurrentStep < USAGE_GUIDE_STEPS.length) {
        usageGuideCurrentStep++;
        runUsageGuideStep(usageGuideCurrentStep);
      }
    });
    ...
    window.addEventListener('resize', () => {
      ...
      if (isFinal) {
        ...
        if (usageGuideNav) {
          positionUsageGuideSpotlight(usageGuideNav);
          positionUsageGuideCard(usageGuideNav);
        }
      } else {
        ...
        if (targetNav) {
          positionUsageGuideSpotlight(targetNav);
          positionUsageGuideCard(targetNav);
        }
      }
    });
  }

  function maybeShowUsageGuideOnFirstEntry() {
  ...
  const USAGE_GUIDE_NO_MORE_KEY = 'usageGuideNoLongerShow';
  ...
  navItems.forEach(item => {
    item.addEventListener('click', (e) => {
      ...
      if (pageId) {
        if (pageId === 'usage-guide') {
          startUsageGuide(true);
          return;
        }
